...
func (v *BlockValidator) ValidateState(block, parent *types.Block, statedb *state.StateDB, receipts types.Receipts, usedGas *big.Int) error {
	...
	log.Debugf("validate block: %d, hash: %x, -------------------------------------------------------------- receipts: %v", block.Number(), block.Hash().Bytes()[:8], receipts)
...
func CalcGasLimit(parent *types.Block) *big.Int {
	...
	log.Debugf("[CalcGasLimit] contrib: %d, decay: %d", contrib, decay)
	...
	*/
	...
	log.Debugf("[CalcGasLimit] gl: %d, minGasLimit: %d", gl, params.MinGasLimit)
	...
	if gl.Cmp(params.TargetGasLimit) < 0 {
		...
		log.Debugf("[CalcGasLimit] gl: %d, TargetGasLimit: %d", gl, params.TargetGasLimit)
	}
	return gl
}
