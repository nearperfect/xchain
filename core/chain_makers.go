...
func GenerateChain(config *params.ChainConfig, parent *types.Block, db mcdb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	...
	for i := 0; i < n; i++ {
		// no need to enable contract metadata if we are just generating fake chain
...
func newCanonical(n int, full bool) (mcdb.Database, *BlockChain, error) {
	...
	blockchain, _ := NewBlockChain(db, params.AllProtocolChanges, ethash.NewFaker(), vm.Config{}, &config.Configuration{})
...
func makeBlockChain(parent *types.Block, n int, db mcdb.Database, seed int) []*types.Block {
	blocks, _ := GenerateChain(params.AllProtocolChanges, parent, db, n, func(i int, b *BlockGen) {
