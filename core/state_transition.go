...
func ApplyMessage(evm *vm.EVM, msg Message, gp *GasPool) ([]byte, *big.Int, bool, error) {
	...
	if msg.GetMsgHash() == nil {
		log.Debugf("[core/state_transition.go->ApplyMessage] msgHash %v, gas used: %d, gaspool: %v", msg.GetMsgHash(), gasUsed, gp)
	} else {
		log.Debugf("[core/state_transition.go->ApplyMessage] msgHash %v, gas used: %d, gaspool: %v", msg.GetMsgHash().Hex(), gasUsed, gp)
	}
...
func (st *StateTransition) TransitionDb() (ret []byte, requiredGas, usedGas *big.Int, failed bool, err error) {
	...
	if contractCreation {
		if msg.GetSystem() > 0 {
			...
			ret, contractAddr, st.gasRemaining, vmerr = evm.Create(
				sender,
				st.data,
				st.gasRemaining,
				st.value,
		...
		log.Debugf(
			"created contract address:%v, leftOverGas:%v, codeHash:%v, ret:%v",
			contractAddr.String(),
			st.gasRemaining,
			evm.StateDB.GetCodeHash(contractAddr).String(),
...
func (st *StateTransition) TransitionDbForCalculateGas() (ret []byte, requiredGas, usedGas *big.Int, failed bool, err error) {
	...
	if contractCreation {
		...
		// Increment the nonce for the next transaction
		st.state.SetNonce(sender.Address(), st.state.GetNonce(sender.Address())+1)
		ret, st.gasRemaining, vmerr = evm.Call(
			sender,
			st.to().Address(),
			st.data,
			st.gasRemaining,
			st.value,
			msg.WaitBlockNumber().Cmp(big.NewInt(0)) == 0,
			msg.ShardFlag(),
			precompiledContracts,
			msgHash,
		)
