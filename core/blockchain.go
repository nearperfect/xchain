...
func (bc *BlockChain) loadLastState() error {
	...
	if head == (common.Hash{}) {
		...
		log.Warn("Invalid blockchain state db, try 'moac recover' to recover")
		return fmt.Errorf("Invalid blockchain state db")
	}
...
// Reset the chain to either of the followings:
...
// Binary scan for valid block with state db in the past
func (bc *BlockChain) BinaryScanForValidBlock() uint64 {
	low, mid, high := uint64(0), uint64(0), uint64(1000000000000) // 1 trillion
	stateThreshold := uint64(10000)
	prev_mid := uint64(0)
	blockFound := uint64(0)
	statedbFound := uint64(0)
	blockDelay := uint64(10000)
	log.Infof(
		"Search for canonical block with valid state db...",
	)

	// first binary search to find the canonical block
	for {
		mid = (low + high) / 2
		if mid == prev_mid {
			break
		}
		log.Infof("block search: low = %d, mid = %d, high = %d", low, mid, high)
		block := bc.GetBlockByNumber(mid)
		if block != nil {
			low = mid
			blockFound = mid
		} else {
	...
	// linear search from blockFound to find the canonical block with valid state db
	if blockFound != 0 {
		// search backwards linearly in range (blockFound, blockFound - blockDelay)
		count := uint64(0)
		for blockNum := blockFound; blockNum > 0 && blockNum > blockFound-blockDelay; blockNum-- {
			block := bc.GetBlockByNumber(blockNum)
			statedb, _ := bc.StateAt(block.Root(), block.Number())
			it := state.NewNodeIterator(statedb)
			count = uint64(0)
			for it.Next() {
				count++
				if count > stateThreshold {
					statedbFound = blockNum
					break
				}
			}
			// found block with valid state db
			if count > stateThreshold {
				statedbFound = blockNum
				log.Infof("state db valid for block: %d", blockNum)
			}
		}
	}
...
func (bc *BlockChain) Insert(block *types.Block, forceUpdateHead bool) {
	bc.insert(block, forceUpdateHead)
}
...
func (bc *BlockChain) insert(block *types.Block, forceUpdateHead bool) {
	...
	if updateHeads || forceUpdateHead {
...
func (bc *BlockChain) WriteBlockAndState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	stateRoot, errState := state.CommitTo(batch, true)
	if errState != nil || stateRoot != block.Root() {
		log.Errorf(
			"State commit diff(block=%d), local root: %x, block root: %x",
			block.NumberU64(), stateRoot, block.Hash(),
		)
...
func (bc *BlockChain) insertChain(chain types.Blocks, liveFlag bool, syncBlock bool) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		log.Infof("validate state(block=%d, txs=%d), root: %x, err: %v", block.NumberU64(), len(block.Transactions()), state.Root(), err)
		if err != nil {
			bc.reportBlock(block, receipts, err)
			return i, events, coalescedLogs, err
		}
		...
		log.Infof(
			"write block & State(block=%d, txs=%d), root: %x, err: %v",
