		...
		case newConfigVersion := <-newConfigChan:
			...
			for _, vaultPairConfig := range sentinel.vaultsConfig.Vaults {
				...
				for _, tokenMapping := range vaultPairConfig.TokenMappings {
					go xdefiContextXY.CommitTokenMints(sentinel, tokenMapping)
					go xdefiContextXY.RejectTokenMints(sentinel, tokenMapping)
				...
				// forward vault events from xchain to target chain
				for _, tokenMapping := range vaultPairConfig.TokenMappings {
					go xdefiContextYX.CommitTokenMints(sentinel, tokenMapping)
					go xdefiContextYX.RejectTokenMints(sentinel, tokenMapping)
