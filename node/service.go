...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (mcdb.Database, error) {
	if ctx.config.DataDir == "" {
		return mcdb.NewMemDatabase()
	}
	db, err := mcdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
