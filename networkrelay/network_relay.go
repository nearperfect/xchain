...
func (self *NetworkRelay) NotifyScsFinalize(curblock *big.Int, liveFlag bool) {
	...
	for _, notify := range notifyScsMsg {
		...
		log.Debugf("NetworkRelay NotifyScsFinalize msgInHex %s", params.VnodePushStatusName(msgInHex))
		...
		var requestId []byte
		if nodeObj != nil {
			requestId, _ = nodeObj.GetRequestId(false)
...
func (self *NetworkRelay) NotifyScs(address common.Address, msg []byte, hash common.Hash, blocknumber *big.Int) {
	// network relay is nil when importing blocks from local backup file
	if self == nil {
		log.Debugf("Notify scs is nil")
		return
	}

	log.Debugf(
		"[mc/network_relay] NotifyScs address %v msg %v hash %v",
		common.Bytes2Hex(address.Bytes()),
		common.Bytes2Hex(msg),
		common.Bytes2Hex(hash.Bytes()),
	)
...
func (self *NetworkRelay) BroadcastMsg(scsmsg *pb.ScsPushMsg, forceToMainnet bool) {
	log.Debugf(
		"[mc/network_relay] BroadcastMsg Requestid: %s, Subchainid: %s, forceToMainnet: %t",
		string(scsmsg.Requestid),
		common.ToHex(scsmsg.Subchainid[:]),
		forceToMainnet,
	)
	if !self.ShouldBroadcast(scsmsg) {
		log.Debugf(
			"[mc/network_relay] Too many msgs type: %v, status: %v",
			common.BytesToInt(scsmsg.Type),
			common.BytesToInt(scsmsg.Status),
		)
		return
	}
	if forceToMainnet {
...
func (self *NetworkRelay) ShouldBroadcast(msg *pb.ScsPushMsg) bool {
	...
	// Pass msg that can be broadcasted
	if (msgType == params.BroadCast &&
		(msgStatus == params.NewBlock ||
			msgStatus == params.NewProposal ||
			msgStatus == params.RNGShares ||
			msgStatus == params.SigShares ||
			msgStatus == params.RequestSigShares)) ||
		(msgType == params.ControlMsg &&
			(msgStatus == params.CreateProposal ||
				msgStatus == params.DisputeProposal ||
				msgStatus == params.ApproveProposal ||
				msgStatus == params.UpdateLastFlushBlk ||
				msgStatus == params.DistributeProposal ||
				msgStatus == params.DistributeProposalAndVSSGroupConfig ||
				msgStatus == params.UploadRedeemData ||
...
func decodeScsPushMsg(raw *pb.ScsPushMsg) DecodedScsPushMsg {
...
func (self *NetworkRelay) GetSCSRole(subchainId, scsId common.Address) params.ScsKind {
	...
	shouldUpdateRoleCache := (roleInfo == nil || roleInfo.Number == 0 || curNumber > roleInfo.Number+maxRoleCache || curNumber < maxRoleCache)
	if shouldUpdateRoleCache {
	...
	if roleInfo != nil {
		log.Debugf("GetSCSRole cache info scsRole:%v, number:%v, curNumber:%v for scsid %x, updated: %t", scsRole, roleInfo.Number, curNumber, scsId, shouldUpdateRoleCache)
	} else {
		log.Debugf("GetSCSRole cache info scsRole:%v, number:0, curNumber:%v for scsid %x, roleInfo is nil, update: %t", scsRole, curNumber, scsId, shouldUpdateRoleCache)
	}
...
func (self *NetworkRelay) ScsMsgCheck(role params.ScsKind, scs common.Address, conReq *pb.ScsPushMsg) bool {
	...
	log.Debugf("[mc/network_relay.go->ScsMsgCheck] role:%v, types:%v, status:%v, scs:%x", role, types, status, scs)
	if types == params.ScsPing || types == params.ScsShakeHand {
		...
		case params.ConsensusScs:
			if types == params.ControlMsg &&
				(status == params.RegOpen ||
					status == params.RegAdd ||
					status == params.RegAsMonitor ||
					status == params.RegAsBackup) {
				return false
			} else {
				return true
			}
		case params.MonitorScs:
			if (types == params.BroadCast &&
				(status != params.RNGShares &&
					status != params.SigShares &&
					status != params.RequestSigShares)) ||
				(types == params.ControlMsg &&
					(status == params.DistributeProposal ||
						status == params.RegAsMonitor ||
		...
		case params.MatchSelTarget:
			if types == params.ControlMsg &&
				(status == params.RegOpen ||
					status == params.RegAdd ||
					status == params.DistributeProposal ||
					status == params.RequestReleaseImmediateAndVSSGroupConfig) {
				return true
			} else {
				return false
			}
...
func (self *NetworkRelay) VnodePushMsg(conReq *pb.ScsPushMsg) (map[int]*pb.ScsPushMsg, error) {
	subchainBaseAddress := common.BytesToAddress(conReq.GetSubchainid())
	log.Debugf("[mc/network_relay.go->VnodePushMsg] called: %v", subchainBaseAddress.String())
	...
	if self != nil && serverList != nil {
		for _, scsConnection := range serverList {
			...
			scsRole := self.GetSCSRole(subchainBaseAddress, common.HexToAddress(scsConnection.ScsId))
			checkResult := self.ScsMsgCheck(scsRole, common.HexToAddress(scsConnection.ScsId), conReq)
			log.Debugf(
				"VnodePushMsg scsRole:%v, checkResult:%v, scs: %s",
				scsRole,
				checkResult,
				scsConnection.ScsId,
			)
