...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/MOACChain/xchain                                              && \
	(cd $GOPATH/src/github.com/MOACChain/xchain && git clone --depth=1 https://github.com/MOACChain/xchain) && \
  go build -v github.com/MOACChain/xchain/cmd/faucet                                              && \
