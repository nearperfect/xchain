...
func (api *PrivateDebugAPI) ActualGas(ctx context.Context, encodedTx hexutil.Bytes) (*big.Int, error) {
	...
	_, gas, err := core.ApplyTransactionForCalculateGas(api.config, api.mc.BlockChain(), nil, new(core.GasPool).AddGas(tx.GasLimit()), st, header, tx, header.GasUsed, vm.Config{}, api.mc.TxPool())
	return gas, err
}
